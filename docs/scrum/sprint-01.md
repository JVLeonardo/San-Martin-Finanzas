...
| S1-02 | Especificación UI/UX para Home y autenticación | Completada documentalmente, pendiente de revisión | Se creó la especificación Figma/UI y el paquete autónomo en inglés `figma-agent/` para Figma Agent: Home responsive, autenticación, tokens, componentes, accesibilidad, estados, referencias y prompt maestro. “Loans” queda como contenido informativo; no hay solicitud de préstamo en Sprint 1. No se modificó código ni se definieron reglas financieras o legales. | Pendiente de commit | Revisión de alcance contra Sprint 1, ADR-002/005/007 y documentación funcional; `git diff --check`; `npm run lint` |
