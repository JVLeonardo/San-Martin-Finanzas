...
| S1-GIT-01 | Flujo Gitflow para el desarrollo frontend | Completada documentalmente, pendiente de revisión | Guía práctica para `dev`, `feature/*`, `release/*` y `hotfix/*`, con Pull Requests mediante GitHub CLI, sincronización de correcciones, versionado semántico y dos rulesets JSON importables para proteger `main` y `dev` durante el trabajo individual. | Pendiente de commit | JSON parseado; targets y checks contrastados con el workflow; comandos y reglas contrastados con documentación oficial de GitHub; `git diff --check`. Pendiente: importar y validar los rulesets en GitHub. |
